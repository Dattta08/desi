    ...
    if str(message.chat.id) in ADMIN_IDS:
        ...
        if len(args) == 3:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve successfully with authorization time in f{duration} days. by @LEGENDVIPOP"
            else:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve successfully with authorization time in {hours} hours. by @LEGENDVIPOP"
            else:
                response = f"Insufficient balance to add user. Required: {cost} Rs. Available: {admin_balances[str(message.chat.id)]} Rs."
        else:
            response = "Days Approval: 👇👇\n\nUsage: /approve <userId> <duration_in_days>\n\nHours Approval 👇👇\n\nUsage: /approve <userId> hours <duration_in_hours>"
    else:
